...
Gui, +AlwaysOnTop
Gui, Add,text,y+15 w250 vDate1 ,% "公历：" SubStr(A_Now,1,4) "年" SubStr(A_Now,5,2) "月"SubStr(A_Now,7,2) "日"
Gui Add, Text, w250 h2 0x10
Gui, Add,text, w250 vLunarDate1,% "农历：" LunarDate[1] (LunarDate[5]?" - " LunarDate[5]:"")
Gui Add, Text, w250 h2 0x10
Gui, Add, Edit, w40 gSijian vSijian
Gui, Add, UpDown, vMyUpDown Range1-24 w40, % SubStr(A_Now,9,2)
Gui, Add,text,x+5 yp+3,时
Gui, Add, CheckBox,x+35 vssb gssb, 立春为界
Gui Add, Text,xm w250 h2 0x10
Gui, Add,text, w250 vLunarDate2,% LunarDate[2]
Gui, Add, MonthCal, vMyCalendar gMyCalendar 
Gui,color,ffffff
Gui, show,,日期转换

ssb:
	GuiControlGet, ssb ,, ssb, Checkbox
	gosub MyCalendar
return
...
Sijian:
	...
	If sj&&MyCalendar
		GuiControl,Text,LunarDate2,% Date2LunarDate(MyCalendar (strlen(sj)=1?0 sj:sj),ssb)[2]
return

MyCalendar:
	If MyCalendar {
		Jq:=GetLunarJq(MyCalendar,1), LunarDate:=Date2LunarDate(MyCalendar (strlen(sj)=1?0 sj:sj),ssb) ;(SubStr(MyCalendar,7,2)=Jq[1]?" ( " Jq[2] " )":"")
		ControlGetText, sj ,Edit1
		GuiControl,Text,Date1,% "公历：" SubStr(MyCalendar,1,4) "年" SubStr(MyCalendar,5,2) "月"SubStr(MyCalendar,7,2) "日"
		GuiControl,Text,LunarDate1,% "农历：" LunarDate[1] (LunarDate[5]?" - " LunarDate[5]:"")
		GuiControl,Text,LunarDate2,% LunarDate[2]
	}
return
